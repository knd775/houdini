...
- `experimentalFieldReactivity` (optional, default: `false`): Enables direct reactive reads such as `Query.data` and plain fragment `data`. In this mode, `Query.variables` contains current inputs and the previous boolean becomes `Query.requiresVariables`. Regenerate Houdini's output and restart the dev server after changing this option.
