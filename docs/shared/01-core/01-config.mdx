...
- `experimentalFieldReactivity` (optional, default: `false`): Enables direct reactive Svelte reads such as `Query.data` and generates readonly arrays in query and fragment result types. Regenerate and restart the dev server after changing it. See [Field-level reactivity](~/loading-data/reactivity) for migration details.
